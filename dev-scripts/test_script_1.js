      ...
      (function () {
        ...
        var holoScene, holoCamera, holoRenderer, holoControls, holoAnimationId;

        function initThreeHologram() {
          var container = document.getElementById("hologram-container");
          if (!container) return;
          container.innerHTML = "";
          ...
          if (typeof THREE === "undefined") {
            ...
            s1.onload = function() {
              ...
              s2.onload = function() { buildHologramScene(container); };
              document.head.appendChild(s2);
            };
            document.head.appendChild(s1);
          } else {
            buildHologramScene(container);
          }
        }

        function buildHologramScene(container) {
          holoScene = new THREE.Scene();
          holoScene.fog = new THREE.FogExp2(0x000000, 0.003);

          var w = container.clientWidth || 800;
          var h = container.clientHeight || 600;
          holoCamera = new THREE.PerspectiveCamera(45, w / h, 1, 1000);
          holoCamera.position.set(0, -120, 100);
          holoCamera.up.set(0, 0, 1);
          ...
          holoControls.maxPolarAngle = Math.PI / 2 - 0.05;
          holoControls.autoRotate = true;
          holoControls.autoRotateSpeed = 0.8;

          // Ground grid
          var gridHelper = new THREE.GridHelper(240, 48, 0x00f3ff, 0x003344);
          gridHelper.rotation.x = Math.PI / 2;
          gridHelper.position.z = -1;
          holoScene.add(gridHelper);

          // Build 3D from the current drawing
          generate3DFromDrawing();
        ...
        function onHoloResize() {
          var container = document.getElementById("hologram-container");
          if (!holoCamera || !holoRenderer || !container) return;
          ...
          holoRenderer.setSize(w, h);
                ...
                function generate3DFromDrawing() {
          // A realistic glowing wireframe building using Three.js
          var buildingGroup = new THREE.Group();
          
          var matLine = new THREE.LineBasicMaterial({ color: 0x00f3ff, transparent: true, opacity: 0.9, linewidth: 2 });
          var matSolid = new THREE.MeshBasicMaterial({ color: 0x001a1a, transparent: true, opacity: 0.5, side: THREE.DoubleSide, blending: THREE.AdditiveBlending, depthWrite: false });
          ...
          // Floor 1
          var f1Geo = new THREE.BoxGeometry(w, h, fH);
          var f1Edge = new THREE.EdgesGeometry(f1Geo);
          var f1Line = new THREE.LineSegments(f1Edge, matLine);
          var f1Solid = new THREE.Mesh(f1Geo, matSolid);
          f1Line.position.z = fH / 2;
          f1Solid.position.z = fH / 2;
          buildingGroup.add(f1Line);
          buildingGroup.add(f1Solid);

          // Floor 2
          var f2Geo = new THREE.BoxGeometry(w, h, fH);
          var f2Edge = new THREE.EdgesGeometry(f2Geo);
          var f2Line = new THREE.LineSegments(f2Edge, matLine);
          var f2Solid = new THREE.Mesh(f2Geo, matSolid);
          f2Line.position.z = fH + fH / 2;
          f2Solid.position.z = fH + fH / 2;
          buildingGroup.add(f2Line);
          buildingGroup.add(f2Solid);

          // Pitched Roof
          // A prism for the roof
          var roofShape = new THREE.Shape();
          roofShape.moveTo(-w/2, 0);
          roofShape.lineTo(w/2, 0);
          roofShape.lineTo(0, 40);
          roofShape.lineTo(-w/2, 0);
          var extrudeSettings = { depth: h, bevelEnabled: false };
          var roofGeo = new THREE.ExtrudeGeometry(roofShape, extrudeSettings);
          // Extrude geometry goes along Z. We need to rotate it to match the building (which has h along Y)
          roofGeo.translate(0, 0, -h/2);
          roofGeo.rotateX(Math.PI / 2);
          
          var roofEdge = new THREE.EdgesGeometry(roofGeo);
          var roofLine = new THREE.LineSegments(roofEdge, matLine);
          var roofSolid = new THREE.Mesh(roofGeo, matSolid);
          roofLine.position.z = fH * 2;
          roofSolid.position.z = fH * 2;
          buildingGroup.add(roofLine);
          buildingGroup.add(roofSolid);
          
          holoScene.add(buildingGroup);

          // Add a pulsing animation to the building
          var time = 0;
          window.buildingGroup = buildingGroup;

          // Violation markers
          if (typeof state !== 'undefined' && state && state.analysis && state.analysis.violations) {
            state.analysis.violations.forEach(function(v, idx) {
              var angle = (idx / 3) * Math.PI * 2;
              var px = Math.cos(angle) * 35;
              var py = Math.sin(angle) * 25;
              var markerGeo = new THREE.SphereGeometry(3, 16, 16);
              var markerMat = new THREE.MeshBasicMaterial({
                color: v.severity === \'CRITICAL\' ? 0xff0000 : v.severity === \'MAJOR\' ? 0xff8800 : 0xffff00,
                transparent: true,
                opacity: 0.9
              });
              var marker = new THREE.Mesh(markerGeo, markerMat);
              marker.position.set(px, py, fH * 2 + 10);
              holoScene.add(marker);

              // Tether line
              var points = [new THREE.Vector3(px, py, 0), new THREE.Vector3(px, py, fH * 2 + 10)];
              var lineGeo = new THREE.BufferGeometry().setFromPoints(points);
              var lineMat2 = new THREE.LineBasicMaterial({ color: 0xff0000, transparent: true, opacity: 0.6 });
              holoScene.add(new THREE.Line(lineGeo, lineMat2));
            });
          }

          holoScene.add(new THREE.AmbientLight(0xffffff, 0.5));
        }

        function animateHolo() {
          holoAnimationId = requestAnimationFrame(animateHolo);
          if (holoControls) holoControls.update();
          if (window.buildingGroup) {
            var time = Date.now() * 0.0015;
            window.buildingGroup.position.z = Math.sin(time) * 5;
            var scale = 1 + Math.sin(time * 2) * 0.02;
            window.buildingGroup.scale.set(scale, scale, scale);
          }
        ...
        function disposeHologram() {
          if (holoAnimationId) cancelAnimationFrame(holoAnimationId);
          window.removeEventListener("resize", onHoloResize);
          if (holoRenderer) {
