...
function geminiVisualCorpus(parsed) {
  const parts = [
    parsed?.documentName,
    parsed?.jurisdiction,
    parsed?.summary,
...
function visualRuleBase(parsed, fallback, payload) {
  const corpus = geminiVisualCorpus(parsed);
...
function normalizeGeminiAnalysis(parsed, fallback, payload) {
  const ruleBase = visualRuleBase(parsed, fallback, payload);
  const visualItems = Array.isArray(parsed.extractedItems)
    ? parsed.extractedItems.filter(Boolean).slice(0, 4).map((item) => `Gemini visual read: ${item}`)
    : [];
  const result = {
    ...ruleBase,
    provider: "Gemini vision + deterministic rules",
    providerMessage: "Gemini read the scanned PDF/image; PRUDENCE converted that visual read into deterministic rule rows and compact markup pins.",
...
async function geminiDocumentAnalysis(payload, fallback) {
  ...
  if (!encodedData || !mimeType.toLowerCase().startsWith("image/") || encodedData.length > 18_000_000) {
    return {
      ...fallback,
      providerMessage: "Gemini is configured; this uploaded file type is using the deterministic local rule engine for stable demo results.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...fallback,
        providerMessage: `Gemini returned HTTP ${response.status}; using deterministic PRUDENCE rule checks.`,
      };
    }
    const data = await response.json();
    const content = data?.candidates?.[0]?.content?.parts?.[0]?.text || "";
    return normalizeGeminiAnalysis(stripJsonText(content), fallback, payload);
  } catch (error) {
    return {
      ...fallback,
      providerMessage: "Gemini analysis was unavailable; using deterministic PRUDENCE rule checks.",
    };
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await geminiDocumentAnalysis(payload, analyzePayload(payload));
